import { CircularFeastBoard, ItemPool, SushiCuisineConfig } from "./core/CircularFeastCore";

const board = new CircularFeastBoard();

board.InitStage(SushiCuisineConfig);
console.log("[Demo] 当前美术资源清单：", board.GetCurrentArtManifest());
board.Gold = 20;
board.StartNewDay();

const mustFindItem = (itemName: string) => {
  const item = ItemPool.find((poolItem) => poolItem.Name === itemName);
